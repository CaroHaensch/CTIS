...
This package offers an R interface for the APIs connected to the Global COVID-19 Trends and Impact Survey, formerly also known as COVID-19 World Symptoms Survey.
...
# remotes::install_github("CaroHaensch/CTIS", force = T)

# Loading (required always)
library(CTIS)
```
...
The same check (available dates) can be done for specific regions in a country. Compared to the country names (in English), the names for the regions are  usually specified in the native language. As an example, we check the  available dates for the region Bavaria ("Bayern") in Germany. 
...
We will look up the indicator Vaccine Acceptance for three dates in May 2021 for Germany. Different weighted and unweighted point and variance estimates can be obtained.
...
While the open data is available without account,  the microdata API requires an account that can be requested here by researchers: https://dataforgood.fb.com/docs/covid-19-symptom-survey-request-for-data-access/
...
And that's it. We thank the team of the University of Maryland and Facebook for providing us with this fantastic data source! We also want to note that we used the tutorials provided on the https://covidmap.umd.edu/ site to create the core of the functions. 
